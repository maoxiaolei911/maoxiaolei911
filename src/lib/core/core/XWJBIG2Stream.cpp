...
struct JBIG2HuffmanTable {
  int val;
  uint prefixLen;
  uint rangeLen;		// can also be LOW, OOB, or EOT
  uint prefix;
};
...
public:
    ...
    void buildTable(JBIG2HuffmanTable *table, uint len);
    
    bool decodeInt(int *x, JBIG2HuffmanTable *table);
    
    uint getByteCounter() { return byteCounter; }
    ...
    void setStream(XWStream *strA) { str = strA; }
...
void JBIG2HuffmanDecoder::buildTable(JBIG2HuffmanTable *table, uint len)
{
  uint i, j, k, prefix;
...
uint JBIG2HuffmanDecoder::readBits(uint n)
{
  uint x, mask, nLeft;
...
uint JBIG2HuffmanDecoder::readBit()
{
  if (bufLen == 0) {
    buf = str->getChar();
    ++byteCounter;
...
uint JBIG2MMRDecoder::get24Bits()
{
  while (bufLen < 24) {
    buf = (buf << 8) | (str->getChar() & 0xff);
    bufLen += 8;
...
void  JBIG2MMRDecoder::skipTo(uint length)
{
...
{
public:
    JBIG2Segment(uint segNumA) { segNum = segNumA; }
    virtual ~JBIG2Segment() {}
  
    uint getSegNum() { return segNum; }
    virtual JBIG2SegmentType getType() = 0;
  
    void setSegNum(uint segNumA) { segNum = segNumA; }
...
{
public:
    JBIG2Bitmap(uint segNumA, int wA, int hA);
    virtual ~JBIG2Bitmap();
        
    void clearPixel(int x, int y) {data[y * line + (x >> 3)] &= 0x7f7f >> (x & 7);}
    void clearToOne();
    void clearToZero();
    void combine(JBIG2Bitmap *bitmap, int x, int y, uint combOp);
    JBIG2Bitmap *copy() { return new JBIG2Bitmap(0, this); }
    
    void duplicateRow(int yDest, int ySrc);
    
    void expand(int newH, uint pixel);
    ...
    JBIG2Bitmap * getSlice(uint x, uint y, uint wA, uint hA);
...
private:
    JBIG2Bitmap(uint segNumA, JBIG2Bitmap *bitmap);
    int w, h, line;
    uchar *data;
};

JBIG2Bitmap::JBIG2Bitmap(uint segNumA, int wA, int hA)
    :JBIG2Segment(segNumA)
{
  ...
  data = (uchar *)malloc((h * line + 1) * sizeof(uchar));
  data[h * line] = 0;
}

JBIG2Bitmap::JBIG2Bitmap(uint segNumA, JBIG2Bitmap *bitmap)
	:JBIG2Segment(segNumA)
{
  w = bitmap->w;
  h = bitmap->h;
...
void JBIG2Bitmap::combine(JBIG2Bitmap *bitmap, int x, int y, uint combOp)
{
...
void JBIG2Bitmap::expand(int newH, uint pixel)
{
  if (newH <= h || line <= 0 || newH >= (INT_MAX - 1) / line) {
    return;
  }
  // need to allocate one extra guard byte for use in combine()
  data = (uchar *)realloc(data, (newH * line + 1) * sizeof(uchar));
  if (pixel) {
...
JBIG2Bitmap * JBIG2Bitmap::getSlice(uint x, uint y, uint wA, uint hA)
{
  JBIG2Bitmap *slice;
  uint xx, yy;
...
{
public:
    JBIG2SymbolDict(uint segNumA, uint sizeA);
    virtual ~JBIG2SymbolDict();
    
    JBIG2Bitmap *getBitmap(uint idx) { return bitmaps[idx]; }
    ...
    void setBitmap(uint idx, JBIG2Bitmap *bitmap) { bitmaps[idx] = bitmap; }
...
JBIG2SymbolDict::JBIG2SymbolDict(uint segNumA, uint sizeA)
    :JBIG2Segment(segNumA)
{
  uint i;
...
{
public:
    JBIG2PatternDict(uint segNumA, uint sizeA);
...
JBIG2PatternDict::JBIG2PatternDict(uint segNumA, uint sizeA)
    :JBIG2Segment(segNumA)
{
    size = sizeA;
    bitmaps = (JBIG2Bitmap **)malloc(size * sizeof(JBIG2Bitmap *));
...
{
public:
    JBIG2CodeTable(uint segNumA, JBIG2HuffmanTable *tableA);
...
JBIG2CodeTable::JBIG2CodeTable(uint segNumA, JBIG2HuffmanTable *tableA)
    :JBIG2Segment(segNumA)
{
  table = tableA;
}
...
XWJBIG2Stream::XWJBIG2Stream(XWStream *strA, 
...
void XWJBIG2Stream::close()
{
  ...
  if (segments) {
    deleteXWList(segments, JBIG2Segment);
    segments = NULL;
  }
  if (globalSegments) {
    deleteXWList(globalSegments, JBIG2Segment);
    globalSegments = NULL;
  }
  dataPtr = dataEnd = NULL;
...
int XWJBIG2Stream::getBlock(char *blk, int size)
{
	int n, i;
...
XWString *XWJBIG2Stream::getPSFilter(int, const char *)
{
    return 0;
}

bool XWJBIG2Stream::isBinary(bool)
{
  return str->isBinary(true);
}

int XWJBIG2Stream::lookChar()
{
  if (dataPtr && dataPtr < dataEnd) {
    return (*dataPtr ^ 0xff) & 0xff;
  }
...
void XWJBIG2Stream::reset()
{
  // read the globals stream
  globalSegments = new XWList();
...
void XWJBIG2Stream::discardSegment(uint segNum)
{
  JBIG2Segment *seg;
  int i;
...
JBIG2Segment *XWJBIG2Stream::findSegment(uint segNum)
{
  JBIG2Segment *seg;
  int i;
...
inline void XWJBIG2Stream::mmrAddPixels(int a1, int blackPixels,
		                             int *codingLine, int *a0i, int w)
{
	if (a1 > codingLine[*a0i]) {
    if (a1 > w) {
      xwApp->error("JBIG2 MMR row is wrong length ");
      a1 = w;
    }
...
inline void XWJBIG2Stream::mmrAddPixelsNeg(int a1, int blackPixels,
		                                int *codingLine, int *a0i, int w)
{
	if (a1 > codingLine[*a0i]) {
    if (a1 > w) {
    ...
    if (a1 < 0) {
      xwApp->error("Invalid JBIG2 MMR code");
      a1 = 0;
    }
...
bool XWJBIG2Stream::readByte(int *x)
{
  int c0;
...
void XWJBIG2Stream::readCodeTableSeg(uint segNum, uint )
{
  JBIG2HuffmanTable *huffTab;
  uint flags, oob, prefixBits, rangeBits;
  ...
  huffDecoder->reset();
  huffTabSize = 8;
  huffTab = (JBIG2HuffmanTable *)malloc(huffTabSize * sizeof(JBIG2HuffmanTable));
  i = 0;
  val = lowVal;
  while (val < highVal) {
    if (i == huffTabSize) {
      huffTabSize *= 2;
      huffTab = (JBIG2HuffmanTable *)realloc(huffTab, huffTabSize * sizeof(JBIG2HuffmanTable));
    }
    huffTab[i].val = val;
 ...
 eofError:
  xwApp->error("Unexpected EOF in JBIG2 stream");
}

void XWJBIG2Stream::readEndOfStripeSeg(uint length)
{
...
void XWJBIG2Stream::readExtensionSeg(uint length)
{
  for (uint i = 0; i < length; ++i) {
    if (curStr->getChar() == EOF) {
      break;
...
JBIG2Bitmap * XWJBIG2Stream::readGenericBitmap(bool mmr, 
                                               int w, 
                                               int h,
				                               int templ, 
				                               bool tpgdOn,
				                               ...
				                               int mmrDataLength)
{
  ...
  uchar *p0, *p1, *p2, *pp;
  uchar *atP0, *atP1, *atP2, *atP3;
  uint buf0, buf1, buf2;
  uint atBuf0, atBuf1, atBuf2, atBuf3;
  ...
  if (mmr) {
    ...
    mmrDecoder->reset();
    if (w > INT_MAX - 2) {
      xwApp->error("Bad width in JBIG2 generic bitmap");
      // force a call to gmalloc(-1), which will throw an exception
      w = -3;
    }
    ...
    codingLine = (int *)malloc((w + 1) * sizeof(int));
	...
	case EOF:
          ...
          break;
	default:
	  xwApp->error("Illegal code in JBIG2 MMR bitmap data");
          mmrAddPixels(w, 0, codingLine, &a0i, w);
	  break;
	}
      }
	...
	xwApp->error("Missing EOFB in JBIG2 MMR bitmap data");
      }
    }
...
JBIG2Bitmap *XWJBIG2Stream::readGenericRefinementRegion(int w, 
	                                                    int h,
					                                    int templ, 
					                                    bool tpgrOn,
...
void XWJBIG2Stream::readGenericRefinementRegionSeg(uint segNum, 
	                                               bool imm,
				                                   bool , 
				                                   uint ,
				                                   uint *refSegs,
				                                   uint nRefSegs)
{
  ...
  uint w, h, x, y, segInfoFlags, extCombOp;
  uint flags, templ, tpgrOn;
  int atx[2], aty[2];
  JBIG2Segment *seg;
  ...
  if (nRefSegs > 1) {
    xwApp->error("Bad reference in JBIG2 generic refinement segment");
    return;
  }
  if (nRefSegs == 1) {
    if (!(seg = findSegment(refSegs[0])) ||
	seg->getType() != jbig2SegBitmap) {
      xwApp->error("Bad bitmap reference in JBIG2 generic refinement segment");
      return;
    }
...
void XWJBIG2Stream::readGenericRegionSeg(uint segNum, 
                                         bool imm,
			                             bool , 
			                             uint length)
{
  JBIG2Bitmap *bitmap;
  uint w, h, x, y, segInfoFlags, extCombOp, rowCount;
  uint flags, mmr, templ, tpgdOn;
  int atx[4], aty[4];
...
void XWJBIG2Stream::readHalftoneRegionSeg(uint segNum, 
                                          bool imm,
			                              bool , 
			                              uint ,
			                              uint *refSegs, 
			                              uint nRefSegs)
{
  ...
  uint w, h, x, y, segInfoFlags, extCombOp;
  uint flags, mmr, templ, enableSkip, combOp;
  uint gridW, gridH, stepX, stepY, patW, patH;
  int atx[4], aty[4];
  ...
  if (w == 0 || h == 0 || w >= INT_MAX / h) {
    xwApp->error("Bad bitmap size in JBIG2 halftone segment");
    return;
  }
  if (gridH == 0 || gridW >= INT_MAX / gridH) {
    xwApp->error("Bad grid size in JBIG2 halftone segment");
    return;
  }

  // get pattern dictionary
  if (nRefSegs != 1) {
    xwApp->error("Bad symbol dictionary reference in JBIG2 halftone segment");
    return;
  }
  if (!(seg = findSegment(refSegs[0])) ||
  ...
  grayImg = (uint *)malloc(gridW * gridH * sizeof(uint));
  memset(grayImg, 0, gridW * gridH * sizeof(uint));
...
bool XWJBIG2Stream::readLong(int *x)
{
  int c0, c1, c2, c3;
...
void XWJBIG2Stream::readPageInfoSeg(uint )
{
  uint xRes, yRes, flags, striping;
...
void XWJBIG2Stream::readPatternDictSeg(uint segNum, uint length)
{
  JBIG2PatternDict *patternDict;
  JBIG2Bitmap *bitmap;
  uint flags, patternW, patternH, grayMax, templ, mmr;
  int atx[4], aty[4];
  uint i, x;
...
void XWJBIG2Stream::readProfilesSeg(uint length)
{
  for (uint i = 0; i < length; ++i) {
    if (curStr->getChar() == EOF) {
      break;
...
void XWJBIG2Stream::readSegments()
{
  uint segNum, segFlags, segType, page, segLength;
  uint refFlags, nRefSegs;
  uint *refSegs;
  int c1, c2, c3;
  uint i;
  ...
  while (readULong(&segNum)) 
  {
    ...
    refSegs = (uint *)malloc(nRefSegs * sizeof(uint));
    if (segNum <= 256) {
    ...
    if (!pageBitmap && ((segType >= 4 && segType <= 7) ||
      ...
      xwApp->error("First JBIG2 segment associated with a page must be a page information segment");
      goto syntaxError;
    }
    ...
    case 62:
      ...
      break;
    default:
      xwApp->error("Unknown segment type in JBIG2 stream");
	...
	xwApp->error("Invalid segment length in JBIG2 stream");
	if (refSegs)
	free(refSegs);
	break;
      }
...
bool XWJBIG2Stream::readSymbolDictSeg(uint segNum, 
                                      uint,
			                          uint *refSegs, 
			                          uint nRefSegs)
{
  ...
  for (i = 0; i < nRefSegs; ++i) {
	...
	if (numInputSyms > UINT_MAX - j) {
	  xwApp->error("Too many input symbols in JBIG2 symbol dictionary");
	  delete codeTables;
	  goto eofError;
	}
	numInputSyms += j;
  ...
  bitmaps = (JBIG2Bitmap **)malloc((numInputSyms + numNewSyms) * sizeof(JBIG2Bitmap *));
  ...
  if (huff) {
    if (huffDH == 0) {
      ...
      if (i >= (uint)codeTables->getLength()) {
	goto codeTableError;
      }
  ...
  if (huff && !refAgg) {
    symWidths = (uint *)malloc(numNewSyms * sizeof(uint));
  }
  ...
  while (i < numNewSyms) {
    ...
    if (dh < 0 && (uint)-dh >= symHeight) {
      xwApp->error("Bad delta-height value in JBIG2 symbol dictionary");
      goto syntaxError;
    }
    symHeight += dh;
    symWidth = 0;
	...
	if (!arithDecoder->decodeInt(&dw, iadwStats)) {
      ...
      if (dw < 0 && (uint)-dw >= symWidth) {
	xwApp->error("Bad delta-height value in JBIG2 symbol dictionary");
	goto syntaxError;
      }
      symWidth += dw;
      if (i >= numNewSyms) {
	xwApp->error("Too many symbols in JBIG2 symbol dictionary");
	goto syntaxError;
      }
	...
	if (refAggNum == 1) {
	  ...
	  if (symID >= numInputSyms + i) {
	    xwApp->error("Invalid symbol ID in JBIG2 symbol dictionary");
	    goto syntaxError;
	  }
  ...
  while (i < numInputSyms + numNewSyms) {
	...
	(ex && j + run > numExSyms)) {
      xwApp->error("Too many exported symbols in JBIG2 symbol dictionary");
      delete symbolDict;
      goto syntaxError;
    }
    if (ex) {
  ...
  if (j != numExSyms) 
  {
    xwApp->error("Too few symbols in JBIG2 symbol dictionary");
    delete symbolDict;
    goto syntaxError;
  }
 ...
 codeTableError:
  xwApp->error("Missing code table in JBIG2 symbol dictionary");
  delete codeTables;
...
JBIG2Bitmap * XWJBIG2Stream::readTextRegion(bool huff, 
                                            bool refine,
			                                int w, 
			                                int h,
			                                uint numInstances,
			                                ...
			                                int *aty)
{
  ...
  while (inst < numInstances) {
	...
	symID = arithDecoder->decodeIAID(symCodeLen, iaidStats);
      ...
      if (symID >= (uint)numSyms) {
	xwApp->error("Invalid symbol number in JBIG2 text region");
      } else {
...
void XWJBIG2Stream::readTextRegionSeg(uint segNum, 
                                      bool imm,
			                          bool , 
			                          uint ,
			                          uint *refSegs, 
			                          uint nRefSegs)
{
  ...
  JBIG2Bitmap **syms;
  uint w, h, x, y, segInfoFlags, extCombOp;
  uint flags, huff, refine, logStrips, refCorner, transposed;
  uint combOp, defPixel, templ;
  int sOffset;
  uint huffFlags, huffFS, huffDS, huffDT;
  uint huffRDW, huffRDH, huffRDX, huffRDY, huffRSize;
  uint numInstances, numSyms, symCodeLen;
  int atx[2], aty[2];
  uint i, k, kk;
  int j;
  ...
  for (i = 0; i < nRefSegs; ++i) {
	...
	codeTables->append(seg);
      ...
      xwApp->error("Invalid segment reference in JBIG2 text region");
      delete codeTables;
      return;
    }
  }
  ...
  if (huff) {
    ...
    symCodeTab = (JBIG2HuffmanTable *)malloc((numSyms + 1) * sizeof(JBIG2HuffmanTable));
    for (i = 0; i < numSyms; ++i) {
      symCodeTab[i].val = i;
 ...
 codeTableError:
  xwApp->error("Missing code table in JBIG2 text region");
  if (codeTables)
  free(codeTables);
  delete syms;
  return;
...
bool XWJBIG2Stream::readUByte(uint *x)
{
  int c0;
...
bool XWJBIG2Stream::readULong(uint *x)
{
  int c0, c1, c2, c3;
...
bool XWJBIG2Stream::readUWord(uint *x)
{
  int c0, c1;
...
void XWJBIG2Stream::resetGenericStats(uint templ, JArithmeticDecoderStats *prevStats)
{
  int size;
...
void XWJBIG2Stream::resetIntStats(int symCodeLen)
{
  iadhStats->reset();
  iadwStats->reset();
  iaexStats->reset();
...
void XWJBIG2Stream::resetRefinementStats(uint templ, JArithmeticDecoderStats *prevStats)
{
  int size;
