...
XWFontFileType1C::XWFontFileType1C(QIODevice * fileA,
		 	                       int    fileFinalA,
		 	                       ulong offsetA,
		 	                       ulong lenA)
...
XWFontFileType1C::XWFontFileType1C(uchar *filedataA, int lenA, bool freeFileDataA)
...
XWFontFileType1C::~XWFontFileType1C()
{
    if (name) 
        delete name;
    
    if (encoding && encoding != type1StandardEncoding && encoding != type1ExpertEncoding) 
    {
        for (int i = 0; i < 256; ++i) 
            delete [] encoding[i];
    ...
    if (charset &&
        charset != type1CISOAdobeCharset &&
        charset != type1CExpertCharset &&
        charset != type1CExpertSubsetCharset) 
    {
        free(charset);
    }
}

void XWFontFileType1C::convertToCIDType0(char *psName, 
                           int *codeMap, 
                           int nCodes,
				                   ...
				                   void *outputStream)
{
  ...
  buf = XWString::format("  /Supplement {0:d} def\n", topDict.supplement);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  (*outputFunc)(outputStream, "end def\n", 8);
  if (topDict.hasFontMatrix) {
    buf = XWString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] def\n",
  ...
  buf = XWString::format("/FontBBox [{0:.4g} {1:.4g} {2:.4g} {3:.4g}] def\n",
  ...
  buf = XWString::format("/GDBytes {0:d} def\n", gdBytes);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  (*outputFunc)(outputStream, "/CIDMapOffset 0 def\n", 20);
  if (topDict.paintType != 0) {
    buf = XWString::format("/PaintType {0:d} def\n", topDict.paintType);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
    buf = XWString::format("/StrokeWidth {0:.4g} def\n", topDict.strokeWidth);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
  }

  // FDArray entry
  buf = XWString::format("/FDArray {0:d} array\n", nFDs);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  for (i = 0; i < nFDs; ++i) {
    buf = XWString::format("dup {0:d} 10 dict begin\n", i);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
	...
	buf = XWString::format("{0:s}{1:d}", j > 0 ? " " : "",
    ...
    if (privateDicts[i].blueScale != 0.039625) {
      buf = XWString::format("/BlueScale {0:.4g} def\n",
			    privateDicts[i].blueScale);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
    if (privateDicts[i].blueShift != 7) {
      buf = XWString::format("/BlueShift {0:d} def\n",
			    privateDicts[i].blueShift);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
    if (privateDicts[i].blueFuzz != 1) {
      buf = XWString::format("/BlueFuzz {0:d} def\n", privateDicts[i].blueFuzz);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
    if (privateDicts[i].hasStdHW) {
      buf = XWString::format("/StdHW [{0:.4g}] def\n", privateDicts[i].stdHW);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
    if (privateDicts[i].hasStdVW) {
      buf = XWString::format("/StdVW [{0:.4g}] def\n", privateDicts[i].stdVW);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
  ...
  offsetA = (nCIDs + 1) * (1 + gdBytes);
  buf = XWString::format("(Hex) {0:d} StartData\n",
			offsetA + charStrings->getLength());
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  ...
  for (i = 0; i <= nCIDs; i += 6) {
	...
	buf2[0] = (char)0;
      ...
      n = offsetA + charStringOffsets[i+j];
      for (k = gdBytes; k >= 1; --k) {
	buf2[k] = (char)(n & 0xff);
	n >>= 8;
      }
      for (k = 0; k <= gdBytes; ++k) {
	buf = XWString::format("{0:02x}", buf2[k] & 0xff);
	(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	delete buf;
      }
    }
  ...
  for (i = 0; i < n; i += 32) {
    for (j = 0; j < 32 && i+j < n; ++j) {
      buf = XWString::format("{0:02x}", charStrings->getChar(i+j) & 0xff);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
    if (i + 32 >= n) {
...
void XWFontFileType1C::convertToType0(char *psName, 
                                      int *codeMap, 
                                      int nCodes,
				                              FontFileOutputFunc outputFunc,
				                              void *outputStream)
{
  ...
  for (i = 0; i < nCIDs; i += 256) {
    ...
    if (j < 256) {
      buf = XWString::format("{0:d} 1 255 {{ 1 index exch /.notdef put }} for\n",
			    j);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
	...
	buf = XWString::format("{0:s}{1:d}", k > 0 ? " " : "",
...
void XWFontFileType1C::convertToType1(char *psName, 
                                      char **newEncoding, 
                                      bool ascii,
		                              FontFileOutputFunc outputFunc, 
		                              void *outputStream)
{
  ...
  buf = XWString::format("/ItalicAngle {0:.4g} def\n", topDict.italicAngle);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  buf = XWString::format("/UnderlinePosition {0:.4g} def\n",
			topDict.underlinePosition);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  buf = XWString::format("/UnderlineThickness {0:.4g} def\n",
  ...
  delete buf;
  (*outputFunc)(outputStream, "/FontType 1 def\n", 16);
  buf = XWString::format("/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] readonly def\n",
			topDict.fontMatrix[0], topDict.fontMatrix[1],
			topDict.fontMatrix[2], topDict.fontMatrix[3],
			topDict.fontMatrix[4], topDict.fontMatrix[5]);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  buf = XWString::format("/FontBBox [{0:.4g} {1:.4g} {2:.4g} {3:.4g}] readonly def\n",
			topDict.fontBBox[0], topDict.fontBBox[1],
			topDict.fontBBox[2], topDict.fontBBox[3]);
  (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
  delete buf;
  ...
  if (topDict.uniqueID != 0) {
    buf = XWString::format("/UniqueID {0:d} def\n", topDict.uniqueID);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
  }
  ...
  if (!newEncoding && encoding == (char **)type1StandardEncoding) {
    ...
    for (i = 0; i < 256; ++i) {
      if (enc[i]) {
	buf = XWString::format("dup {0:d} /{1:s} put\n", i, enc[i]);
	(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	delete buf;
      }
    }
  ...
  if (privateDicts[0].nFamilyOtherBlues) {
    ...
    for (i = 0; i < privateDicts[0].nFamilyOtherBlues; ++i) {
      buf = XWString::format("{0:s}{1:d}", i > 0 ? " " : "",
  ...
  // write the CharStrings
  buf = XWString::format("2 index /CharStrings {0:d} dict dup begin\n",
			nGlyphs);
  eexecWrite(&eb, buf->getCString());
  delete buf;
  for (i = 0; i < nGlyphs; ++i) {
...
int *  XWFontFileType1C::getCIDToGIDMap(int *nCIDs)
{
...
char ** XWFontFileType1C::getEncoding()
{
    return encoding;
}

void XWFontFileType1C::getFontMatrix(double *mat)
{
	int i;
...
XWString * XWFontFileType1C::getGlyphName(int gid)
{
	char buf[256];
  bool ok;
  ...
  return new XWString(buf);
}

char *  XWFontFileType1C::getName()
{
    return name ? name->getCString() : (char *)0;
}

XWFontFileType1C * XWFontFileType1C::load(char *fileName)
{
	QString fn = QFile::decodeName(fileName);
	QFile * fileA = new QFile(fn);
	...
	fileA->close();
	delete fileA;
  XWFontFileType1C * ff = new XWFontFileType1C(buf, lenA, true);
  if (!ff->parse()) 
  {
     delete ff;
     return 0;
...
XWFontFileType1C * XWFontFileType1C::make(char *buf, int lenA)
{
  XWFontFileType1C * ff = new XWFontFileType1C((uchar*)buf, lenA, false);
  if (!ff->parse()) 
  {
     delete ff;
     return 0;
...
void XWFontFileType1C::buildEncoding()
{
  char buf[256];
  ...
  if (topDict.encodingOffset == 0) {
    ...
    encoding = (char **)type1ExpertEncoding;
	...
	encoding[c] = qstrdup(getString(charset[i], buf, &parsedOk));
      }
	...
	for (j = 0; j <= nLeft && nCodes < nGlyphs; ++j) {
	  if (c < 256) {
	    ...
	    encoding[c] = qstrdup(getString(charset[nCodes], buf,
					       &parsedOk));
	  }
	  ++nCodes;
	  ++c;
	...
	encoding[c] = qstrdup(getString(sid, buf, &parsedOk));
      }
    }
  }
}

void XWFontFileType1C::cvtGlyph(int offsetA, 
                                int nBytes, 
                                XWString *charBuf,
		                        Type1CIndex *subrIdx, 
...
void XWFontFileType1C::cvtGlyphWidth(bool useOp, 
                                     XWString *charBuf,
		                             Type1CPrivateDict *pDict)
{
    double w = 0.0;
...
void XWFontFileType1C::cvtNum(double x, bool isFP, XWString *charBuf)
{
  uchar buf[12];
  int y, n;
...
void XWFontFileType1C::eexecCvtGlyph(Type1CEexecBuf *eb, 
                                     char *glyphName,
		                             int offsetA, 
		                             int nBytes,
		                             Type1CIndex *subrIdx,
		                             Type1CPrivateDict *pDict)
{
    XWString * charBuf = new XWString();
    cvtGlyph(offsetA, nBytes, charBuf, subrIdx, pDict, true);

    XWString * buf = XWString::format("/{0:s} {1:d} RD ", glyphName, charBuf->getLength());
    eexecWrite(eb, buf->getCString());
    delete buf;
    eexecWriteCharstring(eb, (uchar *)charBuf->getCString(), charBuf->getLength());
    eexecWrite(eb, " ND\n");

    delete charBuf;
}

void XWFontFileType1C::eexecWrite(Type1CEexecBuf *eb, char *s)
{
  uchar *p;
  uchar x;
...
void XWFontFileType1C::eexecWriteCharstring(Type1CEexecBuf *eb, uchar *s, int n)
{
  uchar x;
  int i;
...
int XWFontFileType1C::getDeltaFPArray(double *arr, int maxLen)
{
  double x;
  int n, i;
...
int XWFontFileType1C::getDeltaIntArray(int *arr, int maxLen)
{
   int x;
  int n, i;
...
void XWFontFileType1C::getIndex(int pos, Type1CIndex *idx, bool *ok)
{
  idx->pos = pos;
...
void XWFontFileType1C::getIndexVal(Type1CIndex *idx, 
	                               int i, 
	                               Type1CIndexVal *val, 
	                               bool *ok)
{
...
int XWFontFileType1C::getOp(int pos, bool charstring, bool *ok)
{
...
char * XWFontFileType1C::getString(int sid, char *buf, bool *ok)
{
  Type1CIndexVal val;
  int n;
  ...
  if (sid < 0) {
	...
	n = 255;
      ...
      strncpy(buf, (char *)&fileData[val.pos], n);
      buf[n] = '\0';
    } else {
      buf[0] = '\0';
    }
...
bool XWFontFileType1C::parse()
{
  ...
  name = new XWString((char *)&fileData[val.pos], val.len);
...
bool XWFontFileType1C::readCharset()
{
  ...
  if (topDict.charsetOffset == 0) {
    ...
    charset = (ushort *)malloc(nGlyphs * sizeof(ushort));
    for (i = 0; i < nGlyphs; ++i) {
      charset[i] = 0;
    }
    ...
    if (charsetFormat == 0) {
      for (i = 1; i < nGlyphs; ++i) {
	charset[i] = (ushort)getU16BE(pos, &parsedOk);
	pos += 2;
	if (!parsedOk) {
	  break;
	}
...
void XWFontFileType1C::readFD(int offsetA, int length, Type1CPrivateDict *pDict)
{
...
void XWFontFileType1C::readFDSelect()
{
  int fdSelectFmt, pos, nRanges, gid0, gid1, fd, i, j;

  fdSelect = (uchar *)malloc(nGlyphs);
  if (topDict.fdSelectOffset == 0) {
	...
	return;
      ...
      memcpy(fdSelect, fileData + pos, nGlyphs);
...
void XWFontFileType1C::readPrivateDict(int offsetA, int length, Type1CPrivateDict *pDict)
{
  int pos;
  ...
  while (pos < offsetA + length) {
	...
	break;
      case 0x0013:
	pDict->subrsOffset = offsetA + (int)ops[0].num;
	break;
      case 0x0014:
...
void XWFontFileType1C::readTopDict()
{
...
void XWFontFileType1C::writePSString(char *s, FontFileOutputFunc outputFunc, void *outputStream)
{
	char buf[80];
  char *p;
  int i, c;
