...
XWFontFileTrueType::XWFontFileTrueType(QIODevice * fileA,
		 	                           int    fileFinalA,
		 	                           ulong offsetA,
		 	                           ulong lenA)
...
XWFontFileTrueType::XWFontFileTrueType(uchar *filedataA, int lenA, bool freeFileDataA)
...
XWFontFileTrueType::~XWFontFileTrueType()
{
    if (tables)
        free(tables);
...
void XWFontFileTrueType::convertToCIDType0(char *psName, 
                           int *cidMap, 
                           int nCIDs,
				                   ...
				                   void *outputStream)
{
  ...
  if (!(ff = XWFontFileType1C::make(start, length))) {
    return;
  }
...
void XWFontFileTrueType::convertToCIDType2(char *psName, 
                                           int *cidMap, 
                                           int nCIDs,
			                               ...
			                               void *outputStream)
{
  ...
  // write the header
  ok = true;
  buf = XWString::format("%!PS-TrueTypeFont-{0:2g}\n",
  ...
  if (cidMap) {
    buf = XWString::format("/CIDCount {0:d} def\n", nCIDs);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
    if (nCIDs > 32767) {
	...
	for (j = 0; j < 32768 - 16 && i+j < nCIDs; j += 16) {
	  ...
	  for (k = 0; k < 16 && i+j+k < nCIDs; ++k) {
	    cid = cidMap[i+j+k];
	    buf = XWString::format("{0:02x}{1:02x}",
				  (cid >> 8) & 0xff, cid & 0xff);
	    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	    delete buf;
	  }
	...
	buf = XWString::format("  {0:d} string 0 1 {1:d} {{\n", 2 * j, j - 1);
	(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	delete buf;
	buf = XWString::format("    2 copy dup 2 mul exch {0:d} add -8 bitshift put\n",
			      i);
	(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	delete buf;
	buf = XWString::format("    1 index exch dup 2 mul 1 add exch {0:d} add"
			      " 255 and put\n", i);
	(*outputFunc)(outputStream, buf->getCString(), buf->getLength());
	delete buf;
	(*outputFunc)(outputStream, "  } for\n", 8);
      ...
      buf = XWString::format("/CIDMap {0:d} string\n", 2 * nGlyphs);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
      buf = XWString::format("  0 1 {0:d} {{\n", nGlyphs - 1);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
      (*outputFunc)(outputStream,
  ...
  buf = XWString::format("/FontBBox [{0:d} {1:d} {2:d} {3:d}] def\n",
...
void XWFontFileTrueType::convertToType0(char *psName, 
                                        int *cidMap, 
                                        int nCIDs,
		                                    bool needVerticalMetrics,
		                                    FontFileOutputFunc outputFunc, 
		                                    void *outputStream)
{
  ...
  sfntsName = (new XWString(psName))->append("_sfnts");
  ...
  for (i = 0; i < n; i += 256) {
    ...
    (*outputFunc)(outputStream, psName, (int)strlen(psName));
    buf = XWString::format("_{0:02x} def\n", i >> 8);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
    (*outputFunc)(outputStream, "/FontType 42 def\n", 17);
    (*outputFunc)(outputStream, "/FontMatrix [1 0 0 1 0 0] def\n", 30);
    ...
    for (j = 0; j < 256 && i+j < n; ++j) {
      buf = XWString::format("dup {0:d} /c{1:02x} put\n", j, j);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
    ...
    (*outputFunc)(outputStream, "/.notdef 0 def\n", 15);
    for (j = 0; j < 256 && i+j < n; ++j) {
      buf = XWString::format("/c{0:02x} {1:d} def\n",
  ...
  for (i = 0; i < n; i += 256) {
    ...
    (*outputFunc)(outputStream, psName, (int)strlen(psName));
    buf = XWString::format("_{0:02x} findfont\n", i >> 8);
    (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
    delete buf;
  }
  (*outputFunc)(outputStream, "] def\n", 6);
...
void XWFontFileTrueType::convertToType1(char *psName, 
                                        char **newEncoding, 
                                        bool ascii,
		                                FontFileOutputFunc outputFunc, 
		                                void *outputStream)
...
void XWFontFileTrueType::convertToType42(char *psName, 
                                         char **encoding,
		                                 int *codeToGID,
		                                 FontFileOutputFunc outputFunc, 
		                                 void *outputStream)
...
int XWFontFileTrueType::findCmap(int platform, int encoding)
{
...
bool XWFontFileTrueType::getCFFBlock(char **start, int *length)
{
	int i;
  ...
  *start = (char *)fileData + tables[i].offset;
  *length = tables[i].len;
  return true;
}

int * XWFontFileTrueType::getCIDToGIDMap(int *nCIDs)
{
  char *start;
  int length;
  XWFontFileType1C *ff;
...
int XWFontFileTrueType::getCmapEncoding(int i)
{
    return cmaps[i].encoding;
}

int  XWFontFileTrueType::getCmapPlatform(int i)
{
    return cmaps[i].platform;
}

int XWFontFileTrueType::getEmbeddingRights()
{
    int i = seekTable("OS/2");
    if (i < 0) 
        return 4;
    
    bool ok = true;
    int fsType = getU16BE(tables[i].offset + 8, &ok);
    if (!ok) 
        return 4;
...
void XWFontFileTrueType::getFontMatrix(double *mat)
{
	char *start;
  int length;
  XWFontFileType1C *ff;
...
int XWFontFileTrueType::getNumCmaps()
{
    return nCmaps;
}

XWFontFileTrueType * XWFontFileTrueType::load(char *fileName)
{
	...
	fileA->close();
	delete fileA;
  XWFontFileTrueType * ff = new XWFontFileTrueType(buf, lenA, true);
  if (!ff->parsedOk) 
  {
     delete ff;
     return 0;
...
XWFontFileTrueType * XWFontFileTrueType::make(char *buf, int lenA)
{
    XWFontFileTrueType * ff = new XWFontFileTrueType((uchar*)buf, lenA, false);
    if (!ff->parsedOk) 
    {
        delete ff;
        return 0;
...
ushort XWFontFileTrueType::mapCodeToGID(int i, int c)
{
  int gid;
...
int XWFontFileTrueType::mapNameToGID(char *name)
{
    if (!nameToGID) 
        return 0;
...
void XWFontFileTrueType::writeTTF(FontFileOutputFunc outputFunc, 
                                  void *outputStream,
		                          char *name, 
		                          ushort *codeToGID)
{
  ...
  uint locaChecksum, glyfChecksum, fileChecksum;
  char *tableDir;
  ...
  if (unsortedLoca) {
	...
	if (checkRegion(pos + k, n)) {
	  glyfChecksum += computeTableChecksum(fileData + pos + k, n);
	}
      }
    }
  }
  ...
  if (name) {
    ...
    newNameTab = (char *)malloc(newNameLen * sizeof(char));
  ...
  if (codeToGID) {
    newCmapLen = 44 + 256 * 2;
    newCmapTab = (char *)malloc(newCmapLen * sizeof(char));
  ...
  if (abbrevHMTX) {
    ...
    pos = tables[i].offset;
    newHHEALen = 36;
    newHHEATab = (char *)malloc(newHHEALen * sizeof(char));
    ...
    newHMTXTab = (char *)malloc(newHMTXLen * sizeof(char));
    advWidth = 0;
  ...
  for (i = 0; i < nTables; ++i) {
	...
	newTables[j].checksum =
	    computeTableChecksum(fileData + tables[i].offset, tables[i].len);
	...
	newTables[j].checksum = computeTableChecksum((uchar *)newCmapTab,
						     newCmapLen);
      } else if (newTables[j].tag == cmapTag && emptyCmap) {
	newTables[j].checksum = computeTableChecksum((uchar *)cmapTab,
						     sizeof(cmapTab));
	...
	newTables[j].checksum = computeTableChecksum((uchar *)newNameTab,
						     newNameLen);
      } else if (newTables[j].tag == hheaTag && abbrevHMTX) {
	newTables[j].len = newHHEALen;
	newTables[j].checksum = computeTableChecksum((uchar *)newHHEATab,
						     newHHEALen);
      } else if (newTables[j].tag == hmtxTag && abbrevHMTX) {
	newTables[j].len = newHMTXLen;
	newTables[j].checksum = computeTableChecksum((uchar *)newHMTXTab,
						     newHMTXLen);
      }
      ++j;
    }
  ...
  if (missingName) {
    ...
    if (name) {
      ...
      newTables[j].checksum = computeTableChecksum((uchar *)nameTab,
						   sizeof(nameTab));
  ...
  if (missingPost) {
    ...
    newTables[j].checksum = computeTableChecksum((uchar *)postTab,
						 sizeof(postTab));
  ...
  if (missingOS2) {
    ...
    newTables[j].checksum = computeTableChecksum((uchar *)os2Tab,
						 sizeof(os2Tab));
  ...
  for (i = -1, t = (uint)nNewTables; t; ++i, t >>= 1) ;
  t = 1 << (4 + i);
  ...
  fileChecksum = computeTableChecksum((uchar *)tableDir,
				      12 + nNewTables * 16);
  for (i = 0; i < nNewTables; ++i) {
  ...
  for (i = 0; i < nNewTables; ++i) {
	...
	(*outputFunc)(outputStream, (char *)fileData + newTables[i].origOffset, 8);
	...
	(*outputFunc)(outputStream,
		      (char *)fileData + newTables[i].origOffset + 12,
		      newTables[i].len - 12);
      } else {
	...
	if (n > 0) {
	  k = locaTable[j].origOffset;
	  if (checkRegion(pos + k, n)) {
	    (*outputFunc)(outputStream, (char *)fileData + pos + k, n);
	  } else {
	    for (k = 0; k < n; ++k) {
	      (*outputFunc)(outputStream, "\0", 1);
	    }
      ...
      if (checkRegion(newTables[i].origOffset, newTables[i].len)) {
	(*outputFunc)(outputStream, (char *)fileData + newTables[i].origOffset,
		      newTables[i].len);
      } else {
	for (j = 0; j < newTables[i].len; ++j) {
	  (*outputFunc)(outputStream, "\0", 1);
...
uint XWFontFileTrueType::computeTableChecksum(int offsetA, int length)
{
	seek(offsetA);
	uchar *data = new uchar[length + 1];
	read((char*)data, length);
	uint ret = computeTableChecksum(data, length);
	delete [] data;
	return ret;
}

uint XWFontFileTrueType::computeTableChecksum(uchar *data, int length)
{
  uint checksum, word;
  int i;
...
void XWFontFileTrueType::cvtCharStrings(char **encoding,
		                                int *codeToGID,
		                                FontFileOutputFunc outputFunc,
		                                void *outputStream)
{
...
void XWFontFileTrueType::cvtEncoding(char **encoding,
		                             FontFileOutputFunc outputFunc,
		                             void *outputStream)
{	
  const char *name;
  ...
  if (encoding) {
	...
	name = ".notdef";
      ...
      buf = XWString::format("dup {0:d} /", i);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
      (*outputFunc)(outputStream, (char*)name, (int)strlen(name));
...
void XWFontFileTrueType::cvtSfnts(FontFileOutputFunc outputFunc,
			                            void *outputStream, 
			                            XWString *name,
			    				                bool needVerticalMetrics,
			    				                int *maxUsedGlyph)
{
  ...
  uchar tableDir[12 + nT42Tables*16];
  bool ok;
  uint checksum;
  int nNewTables;
  ...
  memcpy(headData, fileData + pos, 54);
  headData[8] = headData[9] = headData[10] = headData[11] = (uchar)0;
  ...
  *maxUsedGlyph = -1;
  ...
  locaData = (uchar *)malloc((nGlyphs + 1) * (locaFmt ? 4 : 2) * sizeof(uchar));
  for (i = 0; i <= nGlyphs; ++i) {
    pos = locaTable[i].newOffset;
    if (locaFmt) {
      locaData[4*i  ] = (uchar)(pos >> 24);
      locaData[4*i+1] = (uchar)(pos >> 16);
      locaData[4*i+2] = (uchar)(pos >>  8);
      locaData[4*i+3] = (uchar) pos;
    } else {
      locaData[2*i  ] = (uchar)(pos >> 9);
      locaData[2*i+1] = (uchar)(pos >> 1);
    }
  }
  ...
  for (i = 0; i < nT42Tables; ++i) {
	...
	if (checkRegion(glyfPos + locaTable[j].origOffset, locaTable[j].len)) {
	  checksum +=
	      computeTableChecksum(fileData + glyfPos + locaTable[j].origOffset,
				   locaTable[j].len);
	}
      }
    } else {
	...
	if (checkRegion(tables[j].offset, length)) {
	  checksum = computeTableChecksum(fileData + tables[j].offset, length);
	}
	...
	vmtxTab = (uchar *)malloc(length * sizeof(uchar));
  ...
  tableDir[7] = (uchar)128;
  tableDir[8] = 0;		// entrySelector
  tableDir[9] = 3;
  tableDir[10] = 0;		// rangeShift
  tableDir[11] = (uchar)(16 * nNewTables - 128);
  pos = 12;
  for (i = 0; i < nNewTables; ++i) {
    tableDir[pos   ] = (uchar)(newTables[i].tag >> 24);
    tableDir[pos+ 1] = (uchar)(newTables[i].tag >> 16);
    tableDir[pos+ 2] = (uchar)(newTables[i].tag >>  8);
    tableDir[pos+ 3] = (uchar) newTables[i].tag;
    tableDir[pos+ 4] = (uchar)(newTables[i].checksum >> 24);
    tableDir[pos+ 5] = (uchar)(newTables[i].checksum >> 16);
    tableDir[pos+ 6] = (uchar)(newTables[i].checksum >>  8);
    tableDir[pos+ 7] = (uchar) newTables[i].checksum;
    tableDir[pos+ 8] = (uchar)(newTables[i].offset >> 24);
    tableDir[pos+ 9] = (uchar)(newTables[i].offset >> 16);
    tableDir[pos+10] = (uchar)(newTables[i].offset >>  8);
    tableDir[pos+11] = (uchar) newTables[i].offset;
    tableDir[pos+12] = (uchar)(newTables[i].len >> 24);
    tableDir[pos+13] = (uchar)(newTables[i].len >> 16);
    tableDir[pos+14] = (uchar)(newTables[i].len >>  8);
    tableDir[pos+15] = (uchar) newTables[i].len;
    pos += 16;
  }
  ...
  headData[ 8] = (uchar)(checksum >> 24);
  headData[ 9] = (uchar)(checksum >> 16);
  headData[10] = (uchar)(checksum >>  8);
  headData[11] = (uchar) checksum;
  ...
  for (i = 0; i < nNewTables; ++i) {
	...
	if (locaTable[j].len > 0 &&
	  ...
	  dumpString(fileData + glyfPos + locaTable[j].origOffset,
	...
	if ((j = seekTable(t42Tables[i].tag)) >= 0 &&
	  ...
	  dumpString(fileData + tables[j].offset, tables[j].len,
		     outputFunc, outputStream);
	} else if (needVerticalMetrics && i == t42VheaTable) {
...
void XWFontFileTrueType::dumpString(int offsetA, 
                                    int length,
		                            FontFileOutputFunc outputFunc,
		                            void *outputStream)
{
	seek(offsetA);
	uchar *s = new uchar[length + 1];
	read((char*)s, length);
	dumpString(s, length, outputFunc, outputStream);
	delete [] s;
}

void XWFontFileTrueType::dumpString(uchar *s, 
                                    int length,
		                            FontFileOutputFunc outputFunc,
		                            void *outputStream)
{
  ...
  for (i = 0; i < length; i += 32) {
    for (j = 0; j < 32 && i+j < length; ++j) {
      buf = XWString::format("{0:02x}", s[i+j] & 0xff);
      (*outputFunc)(outputStream, buf->getCString(), buf->getLength());
      delete buf;
    }
...
void XWFontFileTrueType::readPostTable()
{
  XWString *name;
  ...
  if (postFmt == 0x00010000) {
    nameToGID = new XWHash(true);
    for (i = 0; i < 258; ++i) {
      nameToGID->add(new XWString(macGlyphNames[i]), i);
    }
	...
	nameToGID->add(new XWString(macGlyphNames[j]), i);
      } else {
	j -= 258;
	if (j != stringIdx) {
	...
	name = new XWString((char *)&fileData[stringPos + 1], m);
...
int XWFontFileTrueType::seekTable(char *tag)
{
  uint tagI;
  int i;
